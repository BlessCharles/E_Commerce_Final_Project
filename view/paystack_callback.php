...
if (!$reference) {
    ...
    header('Location: book_confirm.php?error=payment_cancelled');
    exit();
}
...
<head>
    ...
    <title>Processing Payment - PlanSmart Ghana</title>
    <style>
        ...
        async function verifyPayment() {
            ...
            try {
                ...
                if (data.status === 'success' && data.verified) {
                    ...
                    setTimeout(() => {
                        window.location.replace(`payment_success.php?reference=${encodeURIComponent(reference)}&event_id=${encodeURIComponent(data.event_id)}&booking_ref=${encodeURIComponent(data.booking_reference)}`);
                    }, 500);
                    ...
                    setTimeout(() => {
                        window.location.href = 'book_confirm.php?error=verification_failed';
                    }, 5000);
                }
                ...
                // Redirect after 5 seconds
                setTimeout(() => {
                    window.location.href = 'book_confirm.php?error=connection_error';
                }, 5000);
            }
        }
